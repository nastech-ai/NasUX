...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.nastech/auth.json active provider)
...
_NASTECH_MODEL = "google/gemini-3-flash-preview"
_NASTECH_DEFAULT_BASE_URL = "https://inference-api.nastechai.com/v1"
...
def _read_nastech_auth() -> Optional[dict]:
    """Read and validate ~/.nastech/auth.json for an active Nous provider.
...
def _resolve_nastech_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        ...
        creds = resolve_nastech_runtime_credentials(
            timeout_seconds=float(os.getenv("NASTECH_NASTECH_TIMEOUT_SECONDS", "15")),
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if runtime is not None:
        ...
        api_key = _nastech_api_key(nous or {})
        if not api_key:
            logger.warning(
                "Auxiliary Nous client unavailable: no usable inference JWT found "
                "(run: nastech auth add nous)."
            )
            _mark_provider_unhealthy("nous", ttl=60)
            return None, None
        base_url = str((nous or {}).get("inference_base_url") or _nastech_base_url()).rstrip("/")
    return (
        OpenAI(
            api_key=api_key,
            base_url=base_url,
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: nastech auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
...
def get_auxiliary_extra_body() -> dict:
    ...
    """
    return _nastech_extra_body() if auxiliary_is_nous else {}
...
def _build_call_kwargs(
    ...
    if provider == "nous" or auxiliary_is_nous:
        merged_extra.setdefault("tags", []).extend(_nastech_portal_tags())
    if merged_extra:
