...
import com.nastech.nasux.shared.nasux.terminal.io.BellHandler;
import com.nastech.nasux.shared.logger.Logger;
import com.nastech.nasux.terminal.TerminalColors;
import com.nastech.nasux.terminal.TerminalSession;
import com.nastech.nasux.terminal.TerminalSessionClient;
import com.nastech.nasux.terminal.TextStyle;
...
public class NasUXTerminalSessionActivityClient extends NasUXTerminalSessionClientBase {

    private final NasUXActivity mActivity;
    ...
    private static final String LOG_TAG = "NasUXTerminalSessionActivityClient";

    public NasUXTerminalSessionActivityClient(NasUXActivity activity) {
    ...
    public void onStart() {
        ...
        if (mActivity.getNasUXService() != null) {
            setCurrentSession(getCurrentStoredSessionOrLast());
            nasuxSessionListNotifyUpdated();
        }
    ...
    public void onSessionFinished(@NonNull TerminalSession finishedSession) {
        ...
        NasUXSession nasuxSession = service.getNasUXSession(index);
        if (nasuxSession != null) {
            isPluginExecutionCommandWithPendingResult = nasuxSession.getExecutionCommand().isPluginExecutionCommandWithPendingResult();
        ...
        if (mActivity.getPackageManager().hasSystemFeature(PackageManager.FEATURE_LEANBACK)) {
            ...
            if (service.getNasUXSessionsSize() > 1 || isPluginExecutionCommandWithPendingResult) {
    ...
    public void onBell(@NonNull TerminalSession session) {
        ...
        switch (mActivity.getProperties().getBellBehaviour()) {
            case NasUXPropertyConstants.IVALUE_BELL_BEHAVIOUR_VIBRATE:
                BellHandler.getInstance(mActivity).doBell();
                break;
            case NasUXPropertyConstants.IVALUE_BELL_BEHAVIOUR_BEEP:
                loadBellSoundPool();
                if (mBellSoundPool != null)
                    mBellSoundPool.play(mBellSoundId, 1.f, 1.f, 1, 0, 1.f);
                break;
            case NasUXPropertyConstants.IVALUE_BELL_BEHAVIOUR_IGNORE:
    ...
    private synchronized void loadBellSoundPool() {
        if (mBellSoundPool == null) {
            ...
            try {
                mBellSoundId = mBellSoundPool.load(mActivity, com.nastech.nasux.shared.R.raw.bell, 1);
            } catch (Exception e){
                // Catch java.lang.RuntimeException: Unable to resume activity {com.nastech.nasux/com.nastech.nasux.app.NasUXActivity}: android.content.res.Resources$NotFoundException: File res/raw/bell.ogg from drawable resource ID
    ...
    public void switchToSession(boolean forward) {
        ...
        int size = service.getNasUXSessionsSize();
        if (forward) {
        ...
        NasUXSession nasuxSession = service.getNasUXSession(index);
        if (nasuxSession != null)
            setCurrentSession(nasuxSession.getTerminalSession());
    }
    ...
    private void renameSession(TerminalSession sessionToRename, String text) {
        ...
        NasUXService service = mActivity.getNasUXService();
        if (service != null) {
            NasUXSession nasuxSession = service.getNasUXSessionForTerminalSession(sessionToRename);
            if (nasuxSession != null)
                nasuxSession.getExecutionCommand().shellName = text;
        }
    }
    ...
    public void addNewSession(boolean isFailSafe, String sessionName) {
        ...
        if (service.getNasUXSessionsSize() >= MAX_SESSIONS) {
            ...
            NasUXSession newNasUXSession = service.createNasUXSession(null, null, null, workingDirectory, isFailSafe, sessionName);
            if (newNasUXSession == null) return;

            TerminalSession newTerminalSession = newNasUXSession.getTerminalSession();
    ...
    public TerminalSession getCurrentStoredSessionOrLast() {
        ...
        if (stored != null) {
            ...
            NasUXSession nasuxSession = service.getLastNasUXSession();
            if (nasuxSession != null)
                return nasuxSession.getTerminalSession();
            else
                return null;
        }
    }
    ...
    public void removeFinishedSession(TerminalSession finishedSession) {
        ...
        int index = service.removeNasUXSession(finishedSession);
    ...
    public void nasuxSessionListNotifyUpdated() {
        mActivity.nasuxSessionListNotifyUpdated();
    }
    ...
    public void checkAndScrollToSession(TerminalSession session) {
        ...
        final ListView nasuxSessionsListView = mActivity.findViewById(R.id.terminal_sessions_list);
        if (nasuxSessionsListView == null) return;

        nasuxSessionsListView.setItemChecked(indexOfSession, true);
        // Delay is necessary otherwise sometimes scroll to newly added session does not happen
        nasuxSessionsListView.postDelayed(() -> nasuxSessionsListView.smoothScrollToPosition(indexOfSession), 1000);
    }
    ...
    public void checkForFontAndColors() {
        try {
            File colorsFile = NasUXConstants.NASUX_COLOR_PROPERTIES_FILE;
            File fontFile = NasUXConstants.NASUX_FONT_FILE;
