...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    nastech_home = _nastech_home_path()
    nastech_root = _nastech_root_path()
    return {
        os.path.realpath(p)
        for p in [
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved NASTECH_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("NASTECH_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    # NasTech control-plane files: block both the ACTIVE profile's view
    # (nastech_home) AND the global root view. Without the root pass, a
    ...
    nastech_dirs = []
    for base in (_nastech_home_path(), _nastech_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in nastech_dirs:
                nastech_dirs.append(real)
        except Exception:
            continue

    for base_real in nastech_dirs:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied NasTech path.
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.nastech/.env`` and exfiltrate the file. The read-deny exists
    ...
    for hd in nastech_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a NasTech credential store "
    ...
    for hd in nastech_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the NasTech MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a NasTech MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Reference: May 2026 incident where a nastech-security profile session
# edited skills under both ``~/.nastech/profiles/nastech-security/skills/``
# AND ``~/.nastech/skills/`` (the default profile's skills) without realizing
...
def _resolve_active_profile_name() -> str:
    ...
    """
    try:
        home_real = _nastech_home_path().resolve()
        root_real = _nastech_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside NasTech scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    NasTech entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to NasTech profile {info['target_profile']!r}, but the "
...
# ``…/sandboxes/<backend>/<task>/home/.nastech/…`` segment and warns
# regardless of which NasTech profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.nastech/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.nastech`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.nastech/…`` and returns the
    index where the inner NasTech-state portion starts. Returns ``None`` for
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative NasTech state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.nastech``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.nastech`` (what the
...
# full ``…/sandboxes/<backend>/<task>/home/.nastech/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.nastech/…``.  The root:root
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative NasTech state.
