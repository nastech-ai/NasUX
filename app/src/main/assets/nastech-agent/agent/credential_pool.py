        ...
        if self.provider == "nous":
            ...
            for token, expires_at in (
                ...
                if (
                    ...
                    and token.strip()
                    and auth_mod._nastech_invoke_jwt_is_usable(
                        token,
...
class CredentialPool:
    ...
    def _sync_nastech_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        """Sync a Nous pool entry from auth.json if tokens differ.

        Nous OAuth refresh tokens are single-use.  When another process
        (e.g. a concurrent cron) refreshes the token via
        ``resolve_nastech_runtime_credentials``, it writes fresh tokens to
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        try:
            if self.provider == "anthropic":
                ...
                synced = self._sync_nastech_entry_from_auth_store(entry)
                if synced is not entry:
                    entry = synced
                auth_mod.resolve_nastech_runtime_credentials(
                    force_refresh=force,
                )
                updated = self._sync_nastech_entry_from_auth_store(entry)
            else:
                return entry
        except Exception as exc:
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For openai-codex: same race as xAI/nous — another NasTech process
            ...
            if self.provider == "nous":
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    return updated
                if auth_mod._is_terminal_nastech_refresh_error(exc):
                    ...
                    try:
                        with _auth_store_lock():
                            ...
                            entry_refresh = str(entry.refresh_token or "").strip()
                            if not store_refresh or store_refresh == entry_refresh:
                                auth_mod._quarantine_nastech_oauth_state(
                                    state,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
                                auth_mod._quarantine_nastech_pool_entries(
                                    auth_store,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
                    ...
                    singleton_sources = {
                        auth_mod.NASTECH_DEVICE_CODE_SOURCE,
                        f"manual:{auth_mod.NASTECH_DEVICE_CODE_SOURCE}",
                    }
                    self._entries = [
                        item for item in self._entries
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # resolve_nastech_runtime_credentials(), making this entry's
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    if provider == "anthropic":
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # (set by persist_nastech_credentials(label=...) when the user ran
            # `nastech auth add nous --label <name>`).  Fall back to the
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            # NasTech PKCE is NasTech-owned/persistable while present, but it is
