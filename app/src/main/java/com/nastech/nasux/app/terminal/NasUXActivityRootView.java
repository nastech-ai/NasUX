...
/**
 * The {@link NasUXActivity} relies on {@link android.view.WindowManager.LayoutParams#SOFT_INPUT_ADJUST_RESIZE)}
 * set by {@link NasUXTerminalViewClient#setSoftKeyboardState(boolean, boolean)} to automatically
 * resize the view and push the terminal up when soft keyboard is opened. However, this does not
 * always work properly. When `enforce-char-based-input=true` is set in `nasux.properties`
 * and {@link com.nastech.nasux.view.TerminalView#onCreateInputConnection(EditorInfo)} sets the inputType
 * to `InputType.TYPE_TEXT_VARIATION_VISIBLE_PASSWORD | InputType.TYPE_TEXT_FLAG_NO_SUGGESTIONS`
 * instead of the default `InputType.TYPE_NULL` for nasux, some keyboards may still show suggestions.
 ...
 * Also check https://github.com/nasux/nasux-app/issues/1539.
 *
 * This overlap may happen even without `enforce-char-based-input=true` for keyboards with extended layouts
 * like number row, etc.
 *
 * To fix these issues, `activity_nasux.xml` has the constant 1sp transparent
 * `activity_nasux_bottom_space_view` View at the bottom. This will appear as a line matching the
 * activity theme. When {@link NasUXActivity} {@link ViewTreeObserver.OnGlobalLayoutListener} is
 ...
 */
public class NasUXActivityRootView extends LinearLayout implements ViewTreeObserver.OnGlobalLayoutListener {

    public NasUXActivity mActivity;
    ...
    private static final String LOG_TAG = "NasUXActivityRootView";

    private static int mStatusBarHeight;

    public NasUXActivityRootView(Context context) {
        super(context);
    }

    public NasUXActivityRootView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public NasUXActivityRootView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    public void setActivity(NasUXActivity activity) {
        mActivity = activity;
    }
    ...
    public void onGlobalLayout() {
        ...
        View bottomSpaceView = mActivity.getNasUXActivityBottomSpaceView();
        ...
        if (isVisible) {
            ...
            // Calling addNasUXActivityRootViewGlobalLayoutListener with a delay fails to
