...
NasTech seeds its credential pool from many places:
    ...
    manual        — user ran `nastech auth add`
...
unify here is **removal**:
    ...
    ``nastech auth remove <provider> <N>`` must make the pool entry stay gone.
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.nastech/.env  → clear it
    ...
    """
    from nastech_cli.config import get_env_path, remove_env_value
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `nastech auth add anthropic` to re-enable if needed.",
    ])


def _remove_nastech_pkce(provider: str, removed) -> RemovalResult:
    """~/.nastech/.anthropic_oauth.json is ours — delete it outright."""
    from nastech_constants import get_nastech_home

    result = RemovalResult()
    oauth_file = get_nastech_home() / ".anthropic_oauth.json"
...
def _remove_nastech_device_code(provider: str, removed) -> RemovalResult:
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``nastech auth remove xai-oauth <N>`` silently undoes
    ...
    Belt-and-braces against the manual entry path: ``nastech auth add
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    via ``nastech auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `nastech auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `nastech auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `nastech auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="nastech_pkce",
        remove_fn=_remove_nastech_pkce,
        description="~/.nastech/.anthropic_oauth.json",
    ))
    register(RemovalStep(
        provider="nous", source_id="device_code",
        remove_fn=_remove_nastech_device_code,
