/**
 * Chinese (Traditional) translations for the NasTech app
 * Values can be:
...
export const zhHant: TranslationStructure = {
    ...
    settings: {
        ...
        about: '關於',
        aboutFooter: 'NasTech 是一個 Codex 和 Claude Code 行動用戶端。它採用端對端加密，您的帳戶僅儲存在本機裝置上。與 Anthropic 無關聯。',
        whatsNew: '更新日誌',
    ...
    errors: {
        ...
        voiceConversationLimitReached: '您本月已達到語音對話的最大次數。我們未來可能會新增按需語音使用功能——如果您遇到此限制，請在 github.com/nastech-ai/nastech/issues 提交 issue。',
    ...
    server: {
        ...
        notValidNasTechServer: '不是有效的 NasTech 伺服器',
        changeServer: '更改伺服器',
    ...
    sessionInfo: {
        ...
        nastechSessionIdCopied: 'NasTech 工作階段 ID 已複製到剪貼簿',
        failedToCopySessionId: '複製 NasTech 工作階段 ID 失敗',
        nastechSessionId: 'NasTech 工作階段 ID',
        ...
        updateCliInstructions: '請執行 pip install -U nastech-agent',
    ...
    settingsVoice: {
        ...
        byoTitle: '使用自己的代理',
        byoDescription: '使用您自己的 ElevenLabs 代理取代 NasTech 預設代理。無需訂閱 — 直接使用您自己的 ElevenLabs 帳戶連線。您的代理必須定義兩個用戶端工具：messageClaudeCode（向編碼代理傳送文字）和 processPermissionRequest（允許或拒絕工具使用）。透過 {{initialConversationContext}} 動態變數接收工作階段上下文。',
        customAgentId: 'ElevenLabs Agent ID',
        customAgentIdNotSet: '未設定',
        customAgentIdDescription: '輸入您的 ElevenLabs Agent ID。留空則使用 NasTech 預設代理。',
        customAgentIdPlaceholder: 'e.g. abc123def456',
        bypassToken: '直接連線',
        bypassTokenSubtitle: '跳過 NasTech 伺服器，直接連線到 ElevenLabs',
    ...
    terminal: {
        ...
        terminalRequestDescription: '有終端正在請求連線到您的 NasTech 帳戶。這將允許終端安全地傳送和接收訊息。',
    ...
    machine: {
        ...
        offlineHelp: '• 確保您的電腦在線上\n• 執行 `nastech daemon status` 進行診斷\n• 您是否在執行最新的 CLI 版本？請使用 `pip install -U nastech-agent` 升級',
        daemon: '守護程序',
        status: '狀態',
        stopDaemon: '停止守護程序',
