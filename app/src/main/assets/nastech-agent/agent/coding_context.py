"""Coding-context awareness — base NasTech, every interactive surface.

When the user runs NasTech inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), NasTech shifts into a **coding posture**. This module is the
...
logger = logging.getLogger("nastech.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "nastech", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _NASTECH_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``NASTECH_TUI_TOOLSETS``, …); they never
