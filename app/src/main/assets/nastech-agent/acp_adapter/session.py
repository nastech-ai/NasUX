...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["nastech-acp"]):
...
class SessionManager:
    ...
    def _get_db(self):
        ...
        Note: we resolve ``NASTECH_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_nastech_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=nastech_home / "state.db")
            return self._db_instance
        except Exception:
