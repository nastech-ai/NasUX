...
Bundled NasTech skills (in `skills/`) ship with every NasTech install. They should be **broadly useful to most users**:
...
If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [NasTech AI Discord](https://discord.gg/NasTech-AI). Users can install it with `nastech skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.nastech/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(nastech_home, config)` for setup-wizard integration
...
- Integrate with `nastech memory setup` via `post_setup()` — no need to touch core code
...
```bash
mkdir -p ~/.nastech/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.nastech/config.yaml
touch ~/.nastech/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.nastech/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/nastech" ~/.local/bin/nastech
...
│
├── nastech_cli/               # CLI command implementations
│   ├── main.py                   # Entry point, argument parsing, command dispatch
│   ├── config.py                 # Config management, migration, env var definitions
│   ├── setup.py                  # Interactive setup wizard
│   ├── auth.py                   # Provider resolution, OAuth, NasTech Portal
...
| `~/.nastech/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.nastech/cron/` | Scheduled job data |
| `~/.nastech/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Provider abstraction**: The agent works with any OpenAI-compatible API. Provider resolution happens at init time (NasTech Portal OAuth, OpenRouter API key, or custom endpoint).
...
Bundled NasTech skills live in `skills/` organized by category. Official optional skills use the same structure in `optional-skills/`:
...
Four fields are supported under `metadata.nastech`:
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `nastech setup` or update `~/.nastech/.env` locally.
...
Create `~/.nastech/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `nastech_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `nastech_cli/skin_engine.py` for the full schema and existing skins as examples.
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the nastech wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `nastech_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `nastech_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `nastech_cli/gateway_windows.py::
...
2. **Test manually**: Run `nastech` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/NasTech-AI/nastech-agent/issues)
...
- **Discord**: [discord.gg/NasTech-AI](https://discord.gg/NasTech-AI) — for questions, showcasing projects, and sharing skills
