...
/**
 * Russian translations for the NasTech app
...
export const ru: TranslationStructure = {
    ...
    settings: {
        ...
        about: 'О программе',
        aboutFooter: 'NasTech — мобильное приложение для работы с Codex и Claude Code. Использует сквозное шифрование, все данные аккаунта хранятся только на вашем устройстве. Не связано с Anthropic.',
        whatsNew: 'Что нового',
    ...
    errors: {
        ...
        voiceConversationLimitReached: 'Вы достигли максимального количества голосовых разговоров в этом месяце. Возможно, в будущем мы добавим голосовое использование по запросу — пожалуйста, создайте заявку на github.com/nastech-ai/nastech/issues, если вы столкнулись с этим ограничением.',
    ...
    server: {
        ...
        notValidNasTechServer: 'Это не валидный сервер NasTech',
    ...
    sessionInfo: {
        ...
        nastechSessionIdCopied: 'ID сессии NasTech скопирован в буфер обмена',
        failedToCopySessionId: 'Не удалось скопировать ID сессии NasTech',
        nastechSessionId: 'ID сессии NasTech',
        ...
        nastechHome: 'Домашний каталог NasTech',
        ...
        updateCliInstructions: 'Пожалуйста, выполните pip install -U nastech-agent',
    ...
    settingsVoice: {
        ...
        byoDescription: 'Используйте собственного агента ElevenLabs вместо стандартного NasTech. Подписка не требуется — подключайтесь напрямую через свой аккаунт ElevenLabs. Ваш агент должен определить два клиентских инструмента: messageClaudeCode (отправляет текст агенту кодирования) и processPermissionRequest (разрешает или запрещает использование инструментов). Контекст сессии передаётся через динамическую переменную {{initialConversationContext}}.',
        customAgentId: 'ElevenLabs Agent ID',
        customAgentIdNotSet: 'Не настроено',
        customAgentIdDescription: 'Введите ваш ElevenLabs Agent ID. Оставьте пустым, чтобы использовать стандартный NasTech.',
        customAgentIdPlaceholder: 'e.g. abc123def456',
        bypassToken: 'Прямое подключение',
        bypassTokenSubtitle: 'Пропустить сервер NasTech, подключиться напрямую к ElevenLabs',
    ...
    terminal: {
        ...
        terminalRequestDescription: 'Терминал запрашивает подключение к вашему аккаунту NasTech. Это позволит терминалу безопасно отправлять и получать сообщения.',
    ...
    machine: {
        ...
        offlineHelp: '• Убедитесь, что компьютер online\n• Выполните `nastech daemon status` для диагностики\n• Используете последнюю версию CLI? Обновите командой `pip install -U nastech-agent`',
