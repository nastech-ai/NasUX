...
``auth.json``.  They intentionally have no dependency on ``nastech_cli.auth`` so
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and NasTech-owned OAuth/device-code state)
