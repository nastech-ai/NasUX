...
def try_recover_primary_transport(
    ...
    if provider_lower in {"nous", "nastech-research"}:
        return False
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    is_nastech_portal = "nastechai" in eff_base_url.lower()
    ...
    if is_native_anthropic:
        return True, True
    if (is_openrouter or is_nastech_portal) and is_claude:
        return True, False
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    if not pre_tool_block_checked:
        try:
            from nastech_cli.plugins import get_pre_tool_call_block_message
