...
``~/.nastech/logs/`` (profile-aware via ``get_nastech_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``nastech_cli.web_server``,
    ``nastech_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from nastech_constants import get_config_path, get_nastech_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_nastech_session_injector", False):
...
# Used by _ComponentFilter and exposed for ``nastech logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "nastech_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the NasTech logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    nastech_home
        Override for the NasTech home directory.  Falls back to
        ``get_nastech_home()`` (profile-aware).
    log_level
    ...
    home = nastech_home or get_nastech_home()
    log_dir = home / "logs"
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the nastech group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from nastech_cli.config import is_managed
