...
Parses x-nastech-credits-* (and optional x-nastech-tool-pool-*) headers from
...
Header schema (x-nastech-credits-* family):
    x-nastech-credits-version                    contract/schema version
    x-nastech-credits-remaining-micros           total remaining balance (micros)
    x-nastech-credits-remaining-usd              same, formatted USD string
    x-nastech-credits-subscription-micros        subscription balance (SIGNED; may be negative/debt)
    x-nastech-credits-subscription-usd           same, formatted USD string
    x-nastech-credits-subscription-limit-micros  subscription cap (PAIRED/optional)
    x-nastech-credits-subscription-limit-usd     same, formatted USD string (PAIRED/optional)
    x-nastech-credits-rollover-micros            rolled-over balance (micros)
    x-nastech-credits-purchased-micros           purchased balance (micros)
    x-nastech-credits-purchased-usd              same, formatted USD string
    x-nastech-credits-denominator-kind           "subscription_cap" | "none"
    x-nastech-credits-paid-access                "true" | "false" (STRING!)
    x-nastech-credits-disabled-reason            reason string (header omitted when null)
    x-nastech-credits-as-of-ms                   server-side timestamp (ms epoch)

Tool-pool headers use a SEPARATE prefix:
    x-nastech-tool-pool-micros                   tool-pool balance (micros)
    x-nastech-tool-pool-gated-off                "true" | "false" (STRING!)
...
@dataclass
class CreditsState:
    """Full credits state parsed from x-nastech-credits-* response headers."""
...
def parse_credits_headers(
    ...
    """Parse x-nastech-credits-* (and x-nastech-tool-pool-*) headers into a CreditsState.

    Returns None (miss) on ANY of:
    - No ``x-nastech-credits-version`` header present.
    ...
    try:
        ...
        version_raw = lowered.get("x-nastech-credits-version")
        ...
        if version_val != 1:
            if version_val > 1 and not _version_warning_emitted:
                ...
                logger.warning(
                    "credits header version %d unsupported, ignoring — update NasTech",
                    version_val,
                )
            return None
        ...
        # ── Parse micros fields ──────────────────────────────────────────────
        remaining_micros = _req_nonneg("x-nastech-credits-remaining-micros")
        if remaining_micros is _SENTINEL:
            return None

        subscription_micros = _req_int("x-nastech-credits-subscription-micros")
        if subscription_micros is _SENTINEL:
            return None

        rollover_micros = _req_nonneg("x-nastech-credits-rollover-micros")
        if rollover_micros is _SENTINEL:
            return None

        purchased_micros = _req_nonneg("x-nastech-credits-purchased-micros")
        ...
        as_of_ms = _req_nonneg("x-nastech-credits-as-of-ms")
        if as_of_ms is _SENTINEL:
            return None

        # ── Validate USD strings ─────────────────────────────────────────────
        remaining_usd = lowered.get("x-nastech-credits-remaining-usd", "")
        ...
        purchased_usd = lowered.get("x-nastech-credits-purchased-usd", "")
        ...
        if "x-nastech-credits-paid-access" in lowered:
            pa_raw = lowered["x-nastech-credits-paid-access"].strip().lower()
        ...
        if "x-nastech-tool-pool-gated-off" in lowered:
            tpgo_raw = lowered["x-nastech-tool-pool-gated-off"].strip().lower()
...
# immediately), and (3) the /usage view (nastech_credits_lines renders the fixture).
# `clear` / `none` / unset → real behaviour. Delete with the rest of the
# NASTECH_DEV_CREDITS scaffolding.
...
def dev_fixture_credits_state() -> Optional[CreditsState]:
    """Return a fixture CreditsState for NASTECH_DEV_CREDITS_FIXTURE, or None.
    ...
    Hard prod-leak guard: a fixture applies ONLY when the dev flag NASTECH_DEV_CREDITS
    is also on, so a stray NASTECH_DEV_CREDITS_FIXTURE (leaked into a shell profile, a
    container env, a launch plist, …) can never surface fabricated balances/notices
    on a real account.
    """
    if not is_truthy_value(os.environ.get("NASTECH_DEV_CREDITS")):
        return None
    raw = os.environ.get("NASTECH_DEV_CREDITS_FIXTURE", "").strip()
    if not raw:
        return None
    name = raw
