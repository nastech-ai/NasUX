...
import com.nastech.nasux.shared.file.filesystem.FileType;
...
import com.nastech.nasux.shared.nasux.NasUXConstants.NASUX_APP.RUN_COMMAND_SERVICE;
 ...
 * plugins that contains info on command execution and forwards the extras to {@link NasUXService}
 * for the actual execution.
 *
 * Check https://github.com/nasux/nasux-app/wiki/RUN_COMMAND-Intent for more info.
 */
public class RunCommandService extends Service {
        ...
        */
        ...
        if (replaceCommaAlternativeCharsInArguments) {
            ...
            if (commaAlternativeCharsInArguments == null)
                commaAlternativeCharsInArguments = NasUXConstants.COMMA_ALTERNATIVE;
            // Replace any commaAlternativeCharsInArguments characters with normal commas
            DataUtils.replaceSubStringsInStringArrayItems(executionCommand.arguments, commaAlternativeCharsInArguments, NasUXConstants.COMMA_NORMAL);
        }
        ...
        // user knows someone tried to run a command in nasux context, since it may be malicious
        ...
        errmsg = NasUXPluginUtils.checkIfAllowExternalAppsPolicyIsViolated(this, LOG_TAG);
        if (errmsg != null) {
            executionCommand.setStateFailed(Errno.ERRNO_FAILED.getCode(), errmsg);
            NasUXPluginUtils.processPluginExecutionCommandError(this, LOG_TAG, executionCommand, true);
            return stopService();
        }
        ...
        executionCommand.executable = NasUXFileUtils.getCanonicalPath(executionCommand.executable, null, true);
        ...
        if (executionCommand.workingDirectory != null && !executionCommand.workingDirectory.isEmpty()) {
            ...
            executionCommand.workingDirectory = NasUXFileUtils.getCanonicalPath(executionCommand.workingDirectory, null, true);
            ...
            // under allowed nasux working directory paths.
            // We try to set execute permissions, but ignore if they are missing, since only read and write permissions are required
            // for working directories.
            error = NasUXFileUtils.validateDirectoryFileExistenceAndPermissions("working", executionCommand.workingDirectory,
                true, true, true,
                false, true);
            if (error != null) {
                executionCommand.setStateFailed(error);
        ...
        executableExtra = NasUXFileUtils.getExpandedNasUXPath(executableExtra);
        ...
        executionCommand.executableUri = new Uri.Builder().scheme(NASUX_SERVICE.URI_SCHEME_SERVICE_EXECUTE).path(executionCommand.executable).build();

        Logger.logVerboseExtended(LOG_TAG, executionCommand.toString());

        // Create execution intent with the action NASUX_SERVICE#ACTION_SERVICE_EXECUTE to be sent to the NASUX_SERVICE
        Intent execIntent = new Intent(NASUX_SERVICE.ACTION_SERVICE_EXECUTE, executionCommand.executableUri);
        execIntent.setClass(this, NasUXService.class);
        execIntent.putExtra(NASUX_SERVICE.EXTRA_ARGUMENTS, executionCommand.arguments);
        execIntent.putExtra(NASUX_SERVICE.EXTRA_STDIN, executionCommand.stdin);
        if (executionCommand.workingDirectory != null && !executionCommand.workingDirectory.isEmpty()) execIntent.putExtra(NASUX_SERVICE.EXTRA_WORKDIR, executionCommand.workingDirectory);
        execIntent.putExtra(NASUX_SERVICE.EXTRA_RUNNER, executionCommand.runner);
        execIntent.putExtra(NASUX_SERVICE.EXTRA_BACKGROUND_CUSTOM_LOG_LEVEL, DataUtils.getStringFromInteger(executionCommand.backgroundCustomLogLevel, null));
        execIntent.putExtra(NASUX_SERVICE.EXTRA_SESSION_ACTION, executionCommand.sessionAction);
        execIntent.putExtra(NASUX_SERVICE.EXTRA_SHELL_NAME, executionCommand.shellName);
        execIntent.putExtra(NASUX_SERVICE.EXTRA_SHELL_CREATE_MODE, executionCommand.shellCreateMode);
        execIntent.putExtra(NASUX_SERVICE.EXTRA_COMMAND_LABEL, executionCommand.commandLabel);
        execIntent.putExtra(NASUX_SERVICE.EXTRA_COMMAND_DESCRIPTION, executionCommand.commandDescription);
        execIntent.putExtra(NASUX_SERVICE.EXTRA_COMMAND_HELP, executionCommand.commandHelp);
        execIntent.putExtra(NASUX_SERVICE.EXTRA_PLUGIN_API_HELP, executionCommand.pluginAPIHelp);
        execIntent.putExtra(NASUX_SERVICE.EXTRA_PENDING_INTENT, executionCommand.resultConfig.resultPendingIntent);
        execIntent.putExtra(NASUX_SERVICE.EXTRA_RESULT_DIRECTORY, executionCommand.resultConfig.resultDirectoryPath);
        if (executionCommand.resultConfig.resultDirectoryPath != null) {
            execIntent.putExtra(NASUX_SERVICE.EXTRA_RESULT_SINGLE_FILE, executionCommand.resultConfig.resultSingleFile);
            execIntent.putExtra(NASUX_SERVICE.EXTRA_RESULT_FILE_BASENAME, executionCommand.resultConfig.resultFileBasename);
            execIntent.putExtra(NASUX_SERVICE.EXTRA_RESULT_FILE_OUTPUT_FORMAT, executionCommand.resultConfig.resultFileOutputFormat);
            execIntent.putExtra(NASUX_SERVICE.EXTRA_RESULT_FILE_ERROR_FORMAT, executionCommand.resultConfig.resultFileErrorFormat);
            execIntent.putExtra(NASUX_SERVICE.EXTRA_RESULT_FILES_SUFFIX, executionCommand.resultConfig.resultFilesSuffix);
        }

        // Start NASUX_SERVICE and pass it execution intent
    ...
    private void runStartForeground() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            startForeground(NasUXConstants.NASUX_RUN_COMMAND_NOTIFICATION_ID, buildNotification());
        }
    }
    ...
    private Notification buildNotification() {
        ...
        Notification.Builder builder =  NotificationUtils.geNotificationBuilder(this,
            NasUXConstants.NASUX_RUN_COMMAND_NOTIFICATION_CHANNEL_ID, Notification.PRIORITY_LOW,
            NasUXConstants.NASUX_RUN_COMMAND_NOTIFICATION_CHANNEL_NAME, null, null,
    ...
    private void setupNotificationChannel() {
        ...
        NotificationUtils.setupNotificationChannel(this, NasUXConstants.NASUX_RUN_COMMAND_NOTIFICATION_CHANNEL_ID,
            NasUXConstants.NASUX_RUN_COMMAND_NOTIFICATION_CHANNEL_NAME, NotificationManager.IMPORTANCE_LOW);
    }
