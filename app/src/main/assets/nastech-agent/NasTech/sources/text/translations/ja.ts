/**
 * Japanese translations for the NasTech app
 * Values can be:
...
export const ja: TranslationStructure = {
    ...
    settings: {
        ...
        about: 'このアプリについて',
        aboutFooter: 'NasTechはCodexとClaude Codeのモバイルクライアントです。完全なエンドツーエンド暗号化を採用し、アカウントはデバイスにのみ保存されます。Anthropicとは提携していません。',
        whatsNew: '新機能',
    ...
    errors: {
        ...
        voiceConversationLimitReached: '今月の音声会話の最大数に達しました。将来的にオンデマンドの音声利用を追加する可能性があります。この制限に達した場合は、github.com/nastech-ai/nastech/issues で issue を作成してください。',
    ...
    server: {
        ...
        notValidNasTechServer: '有効なNasTech Serverではありません',
    ...
    sessionInfo: {
        ...
        nastechSessionIdCopied: 'NasTech Session IDがクリップボードにコピーされました',
        failedToCopySessionId: 'NasTech Session IDのコピーに失敗しました',
        ...
        updateCliInstructions: 'pip install -U nastech-agent を実行してください',
    ...
    settingsVoice: {
        ...
        byoDescription: 'NasTech のデフォルトの代わりに、独自の ElevenLabs エージェントを使用します。サブスクリプション不要 — 自分の ElevenLabs アカウントで直接接続できます。エージェントには2つのクライアントツールを定義する必要があります: messageClaudeCode（コーディングエージェントにテキストを送信）と processPermissionRequest（ツール使用を許可または拒否）。セッションコンテキストは {{initialConversationContext}} 動的変数を通じて受信されます。',
        customAgentId: 'ElevenLabs Agent ID',
        customAgentIdNotSet: '未設定',
        customAgentIdDescription: 'ElevenLabs Agent ID を入力してください。空のままにすると NasTech のデフォルトが使用されます。',
        customAgentIdPlaceholder: 'e.g. abc123def456',
        bypassToken: '直接接続',
        bypassTokenSubtitle: 'NasTech サーバーをスキップし、ElevenLabs に直接接続',
    ...
    terminal: {
        ...
        terminalRequestDescription: 'ターミナルがNasTechアカウントへの接続を要求しています。これにより、ターミナルは安全にメッセージを送受信できるようになります。',
    ...
    machine: {
        ...
        offlineHelp: '• コンピューターがオンラインであることを確認してください\n• `nastech daemon status`を実行して診断してください\n• 最新のCLIバージョンを使用していますか？`pip install -U nastech-agent`でアップグレードしてください',
        daemon: 'デーモン',
        status: 'ステータス',
        stopDaemon: 'デーモンを停止',
