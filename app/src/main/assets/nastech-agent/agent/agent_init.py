...
def _build_codex_gpt55_autoraise_notice(autoraise: Dict[str, float]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: nastech config set compression.codex_gpt55_autoraise false"
    )
...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.nastech/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from nastech_cli.config import load_config as _load_pc_cfg
    ...
    # x-nastech-credits-* response headers after each API call.  Session-start
    # remaining is latched the first time a header is ever seen so we can
    # report cumulative micros spent.  Surfaced behind NASTECH_DEV_CREDITS.
    ...
    # both live under ~/.nastech/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from nastech_logging import setup_logging, setup_verbose_logging
    setup_logging(nastech_home=_ra()._nastech_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that nastech_logging.setup_logging() does not
        ...
        # handler level inside nastech_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from nastech_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `nastech model` to "
                        "select a provider, or run `nastech setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff NASTECH_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["NASTECH_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.nastech/sessions/ alongside gateway sessions
    nastech_home = get_nastech_home()
    agent.logs_dir = nastech_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.nastech/sessions/session_{sid}.json)
    ...
    try:
        from nastech_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from nastech_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "nastech"
                    except Exception:
                        pass
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by NasTech Agent.  Choose a model with at least "
