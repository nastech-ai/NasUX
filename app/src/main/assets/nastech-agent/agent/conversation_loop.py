...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for NasTech tool use: "
    ...
    return (
        ...
        f"context, but NasTech needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In NasTech config, set `model.ollama_num_ctx: 65536` "
...
def _nastech_entitlement_message(capability: str) -> str:
    try:
        ...
        message = format_nastech_portal_entitlement_message(
            account_info,
...
def _print_nastech_entitlement_guidance(agent, capability: str) -> bool:
    message = _nastech_entitlement_message(capability)
    if not message:
        return False
...
def _is_nastech_inference_route(provider: str, base_url: str) -> bool:
    ...
    return (
        base_url_host_matches(base, "inference-api.nastechai.com")
        or base_url_host_matches(base, "inference.nastechai.com")
    )
...
def _billing_or_entitlement_message(
    *,
    ...
    if _is_nastech_inference_route(provider, base_url):
        return _nastech_entitlement_message(capability)
...
def _try_refresh_nastech_paid_entitlement_credentials(agent) -> bool:
    ...
    try:
        ...
        if account_info.paid_service_access is not True:
            return False
        return agent._try_refresh_nastech_client_credentials(
            force=True,
        )
    except Exception:
        return False
...
def run_conversation(
    ...
    # all run inside Codex). Default NasTech path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for NasTech internals.
        #
        # NasTech invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for NasTech tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            ...
            if agent.provider == "nous":
                try:
                    from agent.nastech_rate_guard import (
                        ...
                        format_remaining as _fmt_nastech_remaining,
                    )
                    _nastech_remaining = nastech_rate_limit_remaining()
                    if _nastech_remaining is not None and _nastech_remaining > 0:
                        _nastech_msg = (
                            f"Nous Portal rate limit active — "
                            f"resets in {_fmt_nastech_remaining(_nastech_remaining)}."
                        )
                        agent._buffer_vprint(
                            f"⏳ {_nastech_msg} Trying fallback..."
                        )
                        agent._buffer_status(f"⏳ {_nastech_msg}")
            ...
            try:
                ...
                try:
                    from nastech_cli.middleware import apply_llm_request_middleware
                ...
                from nastech_cli.middleware import run_llm_execution_middleware
                ...
                if (
                    ...
                    and _is_nastech_inference_route(
                        getattr(agent, "provider", "") or "",
                        getattr(agent, "base_url", "") or "",
                    )
                    and not _retry.nastech_paid_entitlement_refresh_attempted
                ):
                    _retry.nastech_paid_entitlement_refresh_attempted = True
                    if _try_refresh_nastech_paid_entitlement_credentials(agent):
                        agent._vprint(
                ...
                if (
                    ...
                    and status_code == 401
                    and not _retry.nastech_auth_retry_attempted
                ):
                    _retry.nastech_auth_retry_attempted = True
                    if agent._try_refresh_nastech_client_credentials(force=True):
                    ...
                    if not _print_nastech_entitlement_guidance(agent, "Nous model access"):
                        print(f"{agent.log_prefix}   Most likely: Portal OAuth expired, account out of credits, or agent key revoked.")
                    print(f"{agent.log_prefix}   Troubleshooting:")
                    print(f"{agent.log_prefix}     • Re-authenticate: nastech auth add nous")
                    print(f"{agent.log_prefix}     • Check credits / billing: https://portal.nastechai.com")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `nastech doctor` for credential-chain diagnostics, or")
                    ...
                    from nastech_constants import display_nastech_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for NasTech-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: nastech config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: nastech config set ANTHROPIC_API_KEY \"\"")
                ...
                # ``is_genuine_nastech_rate_limit`` to tell the two
                ...
                if (
                    ...
                    _genuine_nastech_rate_limit = False
                    try:
                        from agent.nastech_rate_guard import (
                            is_genuine_nastech_rate_limit,
                        ...
                        _genuine_nastech_rate_limit = is_genuine_nastech_rate_limit(
                            headers=_err_hdrs,
                            last_known_state=agent._rate_limit_state,
                        )
                        if _genuine_nastech_rate_limit:
                            record_nastech_rate_limit(
                                headers=_err_hdrs,
                                error_context=error_context,
                            )
                ...
                # which NasTech' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. NasTech' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`nastech",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            pass
                        elif _provider == "nous" and _print_nastech_entitlement_guidance(
                            agent,
                            "Nous model access",
                        ):
                            pass
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `nastech auth` to re-authenticate.", force=True)
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Check your portal account: https://portal.nastechai.com", force=True)
                            ...
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: nastech setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        nastech fallback add   (interactive picker — same as `nastech model`)",
                            force=True,
                        )
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        _policy_response = (
                            ...
                            f"(not a NasTech/gateway failure).\n\n"
                            f"Provider message: {_summary}\n\n"
                            f"Try rephrasing the request, narrowing the context, or "
                            f"adding a fallback provider with `nastech fallback add`."
                        )
                        return {
